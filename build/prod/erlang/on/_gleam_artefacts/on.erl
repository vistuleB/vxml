...
-type return(DPA, DPB) :: {return, DPA} | {stay, DPB}.
...
-spec ok({ok, DPC} | {error, DPD}, fun((DPC) -> {ok, DPG} | {error, DPD})) -> {ok,
        DPG} |
    {error, DPD}.
ok(Result, F2) ->
    case Result of
...
-spec error({ok, DPL} | {error, DPM}, fun((DPM) -> {ok, DPL} | {error, DPP})) -> {ok,
        DPL} |
    {error, DPP}.
error(Result, F2) ->
    case Result of
...
-spec eager_error_ok({ok, DPU} | {error, any()}, DPY, fun((DPU) -> DPY)) -> DPY.
...
-spec error_ok({ok, DPZ} | {error, DQA}, fun((DQA) -> DQD), fun((DPZ) -> DQD)) -> DQD.
...
-spec eager_ok_error({ok, any()} | {error, DQF}, DQI, fun((DQF) -> DQI)) -> DQI.
...
-spec ok_error({ok, DQJ} | {error, DQK}, fun((DQJ) -> DQN), fun((DQK) -> DQN)) -> DQN.
...
-spec eager_false_true(boolean(), DQO, fun(() -> DQO)) -> DQO.
...
-spec false_true(boolean(), fun(() -> DQP), fun(() -> DQP)) -> DQP.
...
-spec eager_true_false(boolean(), DQQ, fun(() -> DQQ)) -> DQQ.
...
-spec true_false(boolean(), fun(() -> DQR), fun(() -> DQR)) -> DQR.
...
-spec some(gleam@option:option(DQS), fun((DQS) -> gleam@option:option(DQU))) -> gleam@option:option(DQU).
some(Option, F2) ->
    case Option of
        none ->
            none;
...
-spec none(gleam@option:option(DQX), fun(() -> gleam@option:option(DQX))) -> gleam@option:option(DQX).
none(Option, F2) ->
    case Option of
        {some, A} ->
            {some, A};
...
-spec eager_none_some(gleam@option:option(DRB), DRD, fun((DRB) -> DRD)) -> DRD.
...
-spec none_some(gleam@option:option(DRE), fun(() -> DRG), fun((DRE) -> DRG)) -> DRG.
...
-spec eager_some_none(gleam@option:option(any()), DRJ, fun(() -> DRJ)) -> DRJ.
...
-spec some_none(gleam@option:option(DRK), fun((DRK) -> DRM), fun(() -> DRM)) -> DRM.
...
-spec nonempty(list(DRN), fun((DRN, list(DRN)) -> list(DRQ))) -> list(DRQ).
nonempty(List, F2) ->
    case List of
        [] ->
            [];
...
-spec empty(list(DRT), fun(() -> list(DRT))) -> list(DRT).
empty(List, F2) ->
    case List of
        [_ | _] ->
            List;
...
-spec eager_empty_nonempty(list(DRX), DRZ, fun((DRX, list(DRX)) -> DRZ)) -> DRZ.
...
-spec empty_nonempty(list(DSB), fun(() -> DSD), fun((DSB, list(DSB)) -> DSD)) -> DSD.
...
-spec eager_nonempty_empty(list(any()), DSH, fun(() -> DSH)) -> DSH.
...
-spec nonempty_empty(list(DSI), fun((DSI, list(DSI)) -> DSL), fun(() -> DSL)) -> DSL.
...
-spec eager_empty_eager_gt1_singleton(list(DTC), DTE, DTE, fun((DTC) -> DTE)) -> DTE.
...
-spec eager_singleton_eager_gt1_empty(list(any()), DTS, DTS, fun(() -> DTS)) -> DTS.
...
-spec stay(return(DUE, DUF), fun((DUF) -> DUE)) -> DUE.
stay(R, F) ->
    case R of
        {return, A} ->
            A;
