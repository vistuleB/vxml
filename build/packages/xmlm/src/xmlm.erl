...
-export([uchar_utf8/1, name_to_string/1, attribute_to_string/1, attributes_to_string/1, tag_to_string/1, signal_to_string/1, signals_to_string/1, input_error_to_string/1, input_stream_from_bit_array/1, input_to_string/1, with_encoding/2, with_stripping/2, with_namespace_callback/2, with_entity_callback/2, from_bit_array/1, from_string/1, eoi/1, peek/1, signal/1, signals/1, fold_signals/3, tree/3, document_tree/3]).
...
-spec fold_signals(input(), NHO, fun((NHO, signal()) -> NHO)) -> {ok,
        {NHO, input()}} |
    {error, input_error()}.
...
-spec tree__loop(
    ...
    list(tag()),
    list(list(NHW)),
    fun((tag(), list(NHW)) -> NHW),
    fun((binary()) -> NHW)
) -> {ok, {NHW, input()}} | {error, input_error()}.
...
-spec tree(input(), fun((tag(), list(NHR)) -> NHR), fun((binary()) -> NHR)) -> {ok,
        {NHR, input()}} |
    {error, input_error()}.
...
-spec document_tree(
    input(),
    fun((tag(), list(NIC)) -> NIC),
    fun((binary()) -> NIC)
) -> {ok, {gleam@option:option(binary()), NIC, input()}} |
    {error, input_error()}.
